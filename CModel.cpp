...
void CModel::DuplicateModel(CModel model)
{
 SetNumberOfPolygons(model.GetNumberOfPolygons());
 for (int i = 0; i < _num_of_polygons; i++)
  SetOnePolygon(i, model.GetPolygon(i));
}
...
void CModel::MoveModelByVector(double angle, CPoint vector)
{
	for (int i = 0; i < _num_of_polygons; i++) 
		_polygons_array[i].MovePolygonByVector(angle, vector);
}
