...
#pragma region Vectors function
void CPoint::MovePointByVector(double angle, CPoint vector)
{
	CVectorRotation _vector;
	CPoint temp;
	temp=_vector.GetNewRotatedPoint(GetX(),GetY(),GetZ(),vector.GetCoordinate(X_AXIS),vector.GetCoordinate(Y_AXIS),vector.GetCoordinate(Z_AXIS),angle);
	_x_coordinate=temp.GetCoordinate(X_AXIS);
	_y_coordinate=temp.GetCoordinate(Y_AXIS);
	_z_coordinate=temp.GetCoordinate(Z_AXIS);
}
#pragma endregion